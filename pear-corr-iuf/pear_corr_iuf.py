...
def form_table():
    ...
    for m in range(1, 1001):        # store the ave rating for each movie
        ...
        if rate_num == 0:           # store the Inverse User Frequency(IUF) for each movie
            iufArr[m] = 1
        else:
            iufArr[m] = np.log(500 / rate_num)
    
    print('iufArr is: ')
    print(iufArr[:10])
    
    for u in range(1, 501):         # store IUFed movie rating for each user
        for m in range(1, 1001):
            if dataArr[u][m] > 0:  
    ...
    for u in range(1, 501):         # store the IUFed ave rating for each user
        ...
        for m in range(1, 1001):
            if iufRateArr[u][m] > 0:
                rate_num += 1
                rate_sum += iufRateArr[u][m]
        iufRateArr[u][0] = rate_sum / rate_num
    ...
    for m in range(1, 1001):        # store the IUFed ave rating for each movie
        ...
        for u in range(1, 501):
            if iufRateArr[u][m] > 0:
                ...
                rate_sum += iufRateArr[u][m]
        if rate_num == 0:
            iufRateArr[0][m] = -1      # nobody rated this movie
        else:
            iufRateArr[0][m] = rate_sum / rate_num
    
    print('dataArr is: ')
    print(dataArr[:10, :10])
    print('iufDataArr is: ')
    print(iufRateArr[:10, :10])
    ...
    for i in range(1, 201):         # calculate the similarities for each old user 1-200
        ...
        for j in range(1, 1001):    # from all the common rated movies
            if new_user[j] > 0 and old_user[j] > 0:
                nume += (new_user_iuf[j] - new_user[0]) * (old_user_iuf[j] - old_user[0])
                sqrN += (new_user_iuf[j] - new_user[0]) ** 2
                sqrO += (old_user_iuf[j] - old_user[0]) ** 2
                cnt += 1
...
def predict(movie_id, sims, idxs, new_user_id):    # calculate the predicted rating by similarity weights and neighbors rating
    ...
    for old_user_id in idxs:
        if dataArr[old_user_id][movie_id] > 0 and np.abs(sims[old_user_id]) > 0.5 and k < 20:
...
f1 = open("PearCor-IUF/PearCor-IUF-result5.txt", "w")       # predictions for result5.txt
...
f2 = open("PearCor-IUF/PearCor-IUF-result10.txt", "w")      # predictions for result10.txt
...
f3 = open("PearCor-IUF/PearCor-IUF-result20.txt", "w")      # predictions for result20.txt
