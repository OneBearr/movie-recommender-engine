...
def form_table():
    ...
    for u in range(1, 501):             # store the IUFed ave rating for each user
        ...
        iufRateArr[u][0] = sum(ratings) / len(ratings)
    ...
    for m in range(1, 1001):            # store the IUFed ave rating for each movie
        ...
        iufRateArr[0][m] = sum(ratings) / len(ratings) if ratings else -1   # -1 if nobody rated this movie
...
def calculate_PearCorIUF_similarities(new_user_id):     # calculate the sims btw the new user and 1-200 old users
    ...
    sims = [0.0]                    # for dummy user0 who does not exist
...
f1 = open("OwnAlgo/OwnAlgo-result5.txt", "w")       # predictions for result5.txt
...
f2 = open("OwnAlgo/OwnAlgo-result10.txt", "w")      # predictions for result10.txt
...
f3 = open("OwnAlgo/OwnAlgo-result20.txt", "w")      # predictions for result20.txt
