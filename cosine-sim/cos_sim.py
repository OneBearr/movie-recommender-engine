...
def calculate_similarities(new_user_id):    # calculate the sims btw the new user and 1-200 old users
    ...
    for i in range(1, 201):         # calculate the similarities for each old user 1-200
        ...
        if cnt <= 1:                # no common rated movie or only one common rated movie
            ...
            sims.append(sim)        # the similarity of the old user i and the new_user
    
    return sims

def find_decreasing_sims_indexes(sims, len):      # find k biggest values' indexes in sims
    return np.argpartition(sims, -len)[-len:][::-1]     
...
f1 = open("CosineSim/CosSim-result5.txt", "w")      # predictions for result5.txt
...
f2 = open("CosineSim/CosSim-result10.txt", "w")     # predictions for result10.txt
...
f3 = open("CosineSim/CosSim-result20.txt", "w")     # predictions for result20.txt
