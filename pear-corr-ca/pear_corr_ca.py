...
def calculate_PearCor_CA_similarities(new_user_id):     # calculate the sims btw the new user and 1-200 old users
    ...
    sims_ca = np.power(np.abs(sims), 1.5) * sims
    return sims_ca
...
def predict(movie_id, sims, idxs, new_user_id):    # calculate the predicted rating by similarity weights and neighbors rating
    nume, deno, res, k = 0.00, 0.00, 0.00, 0
...
f1 = open("pear-corr-ca/pear_corr_ca_result5.txt", "w")       # predictions for result5.txt
for new_user in range(201,301):
    sims = calculate_PearCor_CA_similarities(new_user)         # 1x201 for sim
...
f2 = open("pear-corr-ca/pear_corr_ca_result10.txt", "w")      # predictions for result10.txt
...
f3 = open("pear-corr-ca/pear_corr_ca_result20.txt", "w")      # predictions for result20.txt
