...
def calculate_movie_similarities(movie_id):
    sims = []
    sims.append(0.0)                # for dummy movie0 which does not exist
    for m in range(1, 1001):
        ...
        for u in range(1, 501):
            if dataArr[u][movie_id] > 0 and dataArr[u][m] > 0:
                ...
                # nume = dataArr[u][movie_id] * dataArr[u][m]
                # sqrN = dataArr[u][movie_id] ** 2
                # sqrO = dataArr[u][m] ** 2
                cnt += 1
...
def item_based_predict(sims, idxs, new_user_id, movie_id):
    nume, deno, res = 0.00, 0.00, 0.00
    k = 0
    res = dataArr[new_user_id][0]
    for m in idxs:
        if dataArr[new_user_id][m] > 0 and np.abs(sims[m]) > 0.5 and k < 20:
            nume += sims[m] * (dataArr[new_user_id][m] - dataArr[new_user_id][0])
            deno += np.abs(sims[m])
            # nume += np.abs(sims[m]) * dataArr[new_user_id][m]
            # deno += np.abs(sims[m])
            k += 1
...
for movie_id in range(1, 1001):
    ...
    for new_user in range (201, 501):
        if dataArr[new_user][movie_id] == -1:
            pred = item_based_predict(sims, idxs, new_user, movie_id)
            predArr[new_user][movie_id] = pred
    
    
f1 = open("ItemBased/ItemBased-result5.txt", "w")       # predictions for result5.txt
...
f2 = open("ItemBased/ItemBased-result10.txt", "w")       # predictions for result10.txt
...
f3 = open("ItemBased/ItemBased-result20.txt", "w")       # predictions for result20.txt
